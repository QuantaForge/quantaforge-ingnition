...
it('can solve dusk in production exception', function () {
    ...
    $canSolve = app(RunningQuantaQuirkDuskInProductionProvider::class)->canSolve($exception);
    [$first_solution, $second_solution] = app(RunningQuantaQuirkDuskInProductionProvider::class)->getSolutions($exception);

    expect($canSolve)->toBeTrue();
    expect('QuantaQuirk Dusk should not be run in production.')->toBe($first_solution->getSolutionTitle());
    expect('Install the dependencies with the `--no-dev` flag.')->toBe($first_solution->getSolutionDescription());

    expect('QuantaQuirk Dusk can be run in other environments.')->toBe($second_solution->getSolutionTitle());
