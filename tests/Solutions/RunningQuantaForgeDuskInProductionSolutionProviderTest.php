...
it('can solve dusk in production exception', function () {
    ...
    $canSolve = app(RunningQuantaForgeDuskInProductionProvider::class)->canSolve($exception);
    [$first_solution, $second_solution] = app(RunningQuantaForgeDuskInProductionProvider::class)->getSolutions($exception);

    expect($canSolve)->toBeTrue();
    expect('QuantaForge Dusk should not be run in production.')->toBe($first_solution->getSolutionTitle());
    expect('Install the dependencies with the `--no-dev` flag.')->toBe($first_solution->getSolutionDescription());

    expect('QuantaForge Dusk can be run in other environments.')->toBe($second_solution->getSolutionTitle());
